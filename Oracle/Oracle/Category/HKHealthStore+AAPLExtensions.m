    ...
    HKSampleQuery *query = [[HKSampleQuery alloc] initWithSampleType:quantityType
        ...
        if (completion) {
            ...
            for (HKQuantitySample *qs in results) {
                HKQuantity *q = qs.quantity;
                
                if ([q isCompatibleWithUnit:[HKUnit inchUnit]]) {
                    NSLog(@"All Results INCH : %f",[q doubleValueForUnit:[HKUnit inchUnit]]);
                }
                else if ([q isCompatibleWithUnit:[HKUnit poundUnit]])
                {
                    NSLog(@"All Results POUND : %f",[q doubleValueForUnit:[HKUnit poundUnit]]);
                }
